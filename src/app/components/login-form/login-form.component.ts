  ...
  loginUser(){
    ...
    this.authService.getUserByEmail(username as string).subscribe((response)=>{
      ...
      if(list.length > 0 && list[0].username === username && list[0].role === "dispenser"&& list[0].password === password ){
        this.router.navigate(["dispenserDashboard"]);
      }
      if(list.length > 0 && list[0].username === username && list[0].role === "cashier"&& list[0].password === password ){
        this.router.navigate(["cashierPage"]);
      }
      else{
